...
impl Memory
{
    fn new(size: Option<usize>, regions: Option<Vec<(SectionType, usize, usize)>>) -> Self
    {
        ...
        Self {
            memory: vec![0; clamp(size.unwrap_or(0), std::i32::MAX as usize)],
...
impl Instruction
{
    fn new(mnemonic: u8, modifier: u8, register_from: u8, register_to: u8, data: i32) -> Self { Self { mnemonic, modifier, register_from, register_to, data } }
    ...
    fn parse(mut bytes: Vec<u8>) -> Option<Instruction>
    {
        ...
        for i in 0..bytes.len() {
            ...
            if i == 4 {
                ...
                {
                    xinstruction.data = i32::from_be_bytes(data);
                    break;
                } else { return None; }
            }
    ...
    {
        match self.modifier {
            // value to register, 01 04 00 [??] [00 00 00 00]
            0x1 => {
                if self.register_to < ctx.registers.general.len() as u8 {
                    ctx.registers.general[self.register_to as usize] = self.data as i32;
                } else {
                    return Err(format!("error: invalid register {:02X} at 0x{:X}", self.register_to, ctx.registers.rip + 3))  
                }
            },
            // register to register 01 02 [??] [??] 00 00 00 00
            0x2 => {
                if self.register_to < ctx.registers.general.len() as u8 { 
                    return Err(format!("error: invalid register {:02X} at 0x{:X}", self.register_to, ctx.registers.rip + 3));
                }

                if self.register_from < ctx.registers.general.len() as u8 {
                    return Err(format!("error: invalid register {:02X} at 0x{:X}", self.register_from, ctx.registers.rip + 2));
                }

                ctx.registers.general[self.register_from as usize] = ctx.registers.general[self.register_to as usize];
            },
            // register to memory, 01 03 [??] 00 [00 00 00 00]
            0x3 => {
                if (self.data as usize) < ctx.ram.memory.len() {
                    let bytes = i32::to_be_bytes(ctx.registers.general[self.register_from as usize]);
                    for i in 0..4 { ctx.ram.memory[self.data as usize] = bytes[i]; }
                } else {
                    return Err(format!("error: address size at 0x{:X} is too large", ctx.registers.rip + 4));
                }
            }, 
...
fn main() 
{
    ...
    example.append(Instruction::new(1, 1, 0, 1, 1337));
